...
class Ethna_ActionForm
{
    ...
    function setFormVars()
    {
        ...
        foreach ($this->form as $name => $def) {
            ...
            if ($type == VAR_TYPE_FILE) {
                ...
                // 配列構造の検査
                if (is_array($def['type'])) {
                    if (!empty($http_vars[$name]) && is_array($http_vars[$name]) == false) {
        ...
        for ($i = 0; $i < strlen($string); $i++) {
            ...
            if ($c < 0x80) {
                ...
                return $this->ae->add($name,
                    $this->getErrorMessage('に機種依存文字が入力されています'), E_FORM_INVALIDCHAR);
            } else {
                $i++;
            }
        }
    ...
    function &checkBoolean($name)
    {
        ...
        foreach ($form_vars as $v) {
            ...
            if ($v != "0" && $v != "1") {
                return $this->ae->add($name,
                    $this->getErrorMessage('を正しく入力してください'), E_FORM_INVALIDCHAR);
            }
        }
    ...
    function handleError($name, $code)
    {
        ...
        if ($code == E_FORM_REQUIRED) {
            ...
            case FORM_TYPE_SUBMIT:
                $message = $this->getErrorMessage('ethna_plugin_validator_required_002');
                break;
            case FORM_TYPE_SELECT:
            case FORM_TYPE_RADIO:
            case FORM_TYPE_CHECKBOX:
            case FORM_TYPE_FILE:
                $message = $this->getErrorMessage('ethna_plugin_validator_required_001');
                break;
            default:
                $message = $this->getErrorMessage('ethna_plugin_validator_required_002');
                break;
            ...
            $message = $this->getErrorMessage('ethna_plugin_validator_type_006');
        } else if ($code == E_FORM_WRONGTYPE_ARRAY) {
            $message = $this->getErrorMessage('ethna_plugin_validator_type_005');
        } else if ($code == E_FORM_WRONGTYPE_INT) {
            $message = $this->getErrorMessage('ethna_plugin_validator_type_001');
        } else if ($code == E_FORM_WRONGTYPE_FLOAT) {
            $message = $this->getErrorMessage('ethna_plugin_validator_type_002');
        } else if ($code == E_FORM_WRONGTYPE_DATETIME) {
            $message = $this->getErrorMessage('ethna_plugin_validator_type_004');
        } else if ($code == E_FORM_WRONGTYPE_BOOLEAN) {
            $message = $this->getErrorMessage('ethna_plugin_validator_type_003');
        } else if ($code == E_FORM_MIN_INT) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_min_001'),
                $code, $def['min']);
            return;
        } else if ($code == E_FORM_MIN_FLOAT) {
            $this->ae->add($name,
               $this->getErrorMessage('ethna_plugin_validator_min_002'),
                $code, $def['min']);
            return;
        } else if ($code == E_FORM_MIN_DATETIME) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_min_003'),
                $code, $def['min']);
            return;
        } else if ($code == E_FORM_MIN_FILE) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_min_004'),
                $code, $def['min']);
            return;
        } else if ($code == E_FORM_MIN_STRING) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_min_005'),
                $code, intval($def['min']/2), $def['min']);
            return;
        } else if ($code == E_FORM_MAX_INT) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_max_001'),
                $code, $def['max']);
            return;
        } else if ($code == E_FORM_MAX_FLOAT) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_max_002'),
                $code, $def['max']);
            return;
        } else if ($code == E_FORM_MAX_DATETIME) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_max_003'),
                $code, $def['max']);
            return;
        } else if ($code == E_FORM_MAX_FILE) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_max_004'),
                $code, $def['max']);
            return;
        } else if ($code == E_FORM_MAX_STRING) {
            $this->ae->add($name,
                $this->getErrorMessage('ethna_plugin_validator_max_005'),
                $code, intval($def['max']/2), $def['max']);
            return;
        } else if ($code == E_FORM_REGEXP) {
            $message = $this->getErrorMessage('ethna_plugin_validator_regexp_001');
        }

        $this->ae->add($name, $message, $code);
    }
	
	private function getErrorMessage($key, $plugin = null)
	{
		return "{form}" . smarty_modifier_lang($key, $plugin);
	}
