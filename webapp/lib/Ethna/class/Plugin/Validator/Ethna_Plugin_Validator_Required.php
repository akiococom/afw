        ...
        if (is_array($form_def['type']) == false) {
            if ($this->isEmpty($var, $this->getFormType($name))) {
                if (isset($params['error'])) {
                    ...
                    $msg = '{form}' . $glexalang['glexa']['ethna_plugin_validator_required_001'];
                } else {
                    $msg = '{form}' . $glexalang['glexa']['ethna_plugin_validator_required_002'];
                }
        ...
        if (isset($params['key'])) {
            ...
            if (count($invalid_keys) > 0) {
                if (isset($params['error'])) {
                    ...
                    $msg = '{form}' . $glexalang['glexa']['ethna_plugin_validator_required_003'];
                } else {
                    $msg = '{form}' . $glexalang['glexa']['ethna_plugin_validator_required_004'];
                }
                return Ethna::raiseNotice($msg, E_FORM_REQUIRED);
            }
        }
        ...
        if (isset($params['num'])) {
            if (count($valid_keys) < intval($params['num'])) {
                if (isset($params['error'])) {
                    ...
                    $msg = '{form}' . $glexalang['glexa']['ethna_plugin_validator_required_005'];
                } else {
                    $msg = '{form}' . $glexalang['glexa']['ethna_plugin_validator_required_006'];
                }
                return Ethna::raiseNotice($msg, E_FORM_REQUIRED);
            }
        }
